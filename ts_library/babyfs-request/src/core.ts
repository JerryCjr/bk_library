...
import qs from 'qs';
import PartialInterCeptorContextFactory, { PartialInterceptorManager } from './partial-interceptor';
import { filterRequestEmptyParams } from './util';
import { ApiError } from './api-error';
import { traceRecord, recombineRequest, recombineResponse } from './trace/util';
...
const getInstanceConfig = (instanceConfig: instanceConfig = {}) => {
  const { timeout, baseURL } = instanceConfig;
  return Object.assign({}, defaultRequestConfig, { timeout, baseURL });
};

const AjaxRequestWrapper: Function = function (instanceConfig: instanceConfig) {
  ...
  const partialInterceptorProcessorCreator = () => {
    let reqInterceptorIds: Array<any> = [];
    let resInterceptorIds: Array<any> = [];
    interface item {
      [propName: string]: any;
    }
    const before = (reqInterceptors: any, resInterceptors: any) => {
      reqInterceptorIds = reqInterceptors.map((item: item) => {
        return ajax.interceptors.request.use(item.fulfilled, item.rejected);
      });
      resInterceptorIds = resInterceptors.map((item: item) => {
        return ajax.interceptors.response.use(item.fulfilled, item.rejected);
      });
    };
    const after = () => {
      reqInterceptorIds.forEach((itemId: number) => {
        ajax.interceptors.request.eject(itemId);
      });
      resInterceptorIds.forEach((itemId: number) => {
  ...
  class AjaxRequest {
    registerRequestInterceptor(fulfilled: any, rejected: any) {
      if (fulfilled || rejected) {
        ajax.interceptors.request.use(fulfilled, rejected);
      }
    }

    registerResponseInterceptor(fulfilled: any, rejected: any) {
      if (fulfilled || rejected) {
        ajax.interceptors.response.use(fulfilled, rejected);
      }
    }

    registerPartialRequestInterceptor(contextName: string, fulfilled: any, rejected: any) {
    ...
    registerPartialResponseInterceptor(contextName: string, fulfilled: any, rejected: any) {
    ...
    createInstance(instanceConfig: instanceConfig) {
     ...
     * @param {Object} config
     */
    async get(url: any = urlCanNotBeEmpty(), params: any, { withCredentials = true, filterEmptyParams = true } = {}) {
     ...
     */
    async post(
      url: any = urlCanNotBeEmpty(),
      params: any,
      { withCredentials = true, filterEmptyParams = true, contentType = RequestContentType.URL_ENCODE } = {},
    ) {
     ...
     */
    switch({ returnRawRes = false, traceSw = false } = {}) {
