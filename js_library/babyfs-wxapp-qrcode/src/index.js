 ...
 * @param {number} argv.width 生成二维码的宽
 * @param {number} argv.height 生成二维码的高
 * @param {number} argv.x 坐标x
 * @param {number} argv.y 坐标y
 * @param {object} argv._this 在自定义组件下，当前组件实例的this，表示在这个自定义组件下查找拥有 canvas-id 的 <canvas> ，如果省略则不在任何自定义组件内查找 可选
 * @param {object} argv.ctx 直接传入canvas上下文 可选
 * @param {string} argv.canvasId 要获取上下文的 <canvas> 组件 canvas-id 属性 可选
 * @param {number} argv.typeNumber 二维码的计算模式，默认值-1 可选
 * @param {number} argv.errorCorrectionLevel 二维码纠错级别 默认值为高级 取值：{ L: 1, M: 0, Q: 3, H: 2 } 可选
