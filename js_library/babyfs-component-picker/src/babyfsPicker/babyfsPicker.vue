    ...
    <transition name="slide-top">
      ...
      <div class="babyfs-picker-box" @click.stop v-if="pickerIf && mode === 'region'">
        ...
        <div class="picker-box-contain" :style="'height:'+ pickHeight">
          <div class="box-contain-ul" :class="provinceIf ? 'contain-ul-show' : '' " ref="provinceRef" v-show="provinceIf">
          ...
          <div class="box-contain-ul" :class="cityIf ? 'contain-ul-show' : '' " ref="cityRef" v-show="cityIf">
          ...
          <div class="box-contain-ul" :class="areaIf ? 'contain-ul-show' : '' " ref="areaRef" v-show="areaIf">
  ...
  methods: {
    ...
    init() {
      if (this.mode === 'region') {
        ...
        this.checkSession();
        let height = this.row && this.row > 0 && this.row <= 7 ? (this.row - 0) * 45 + 5 : 330;
        this.pickHeight = `${height}px`;
        this.chooseColor = this.theme ? this.theme : '#FF5E4C';
      } else {
        this.normalInitial();
        let height = this.row && this.row > 0 && this.row <= 7 ? (this.row - 0) * 45 + 5 : '';
        this.pickHeight = `${height}px`;
        this.chooseColor = this.theme ? this.theme : '#FF5E4C';
      }
    },
    ...
    commonRegoin(data, flag) {
      ...
      if (!!this.initial && this.initial.areaCode) {
        getAllParentAreasByCode({areacode: this.initial.areaCode}).then(res => {
          let r = res.data;
          this.$emit('handle', {
            provinceCode: r.c,
            province: r.n,
            cityCode: r.ns[0].ns ? r.ns[0].c : r.c,
            city: r.ns[0].ns ? r.ns[0].n : r.n,
            areaCode: r.ns[0].ns ? r.ns[0].ns[0].c : r.ns[0].c,
            area: r.ns[0].ns ? r.ns[0].ns[0].n : r.ns[0].n
          });
          // ----------------
          if (!r.ns[0].ns || r.ns[0].ns.length === 0) { //直辖市类
            this.isDirect = true;
            this.cityCode = r.ns[0].c;
            this.city = r.ns[0].n;
            this.areaCode = '';
            this.area = '';
          } else {
            ...
            this.cityCode = r.ns[0].c;
            this.city = r.ns[0].n;
            this.areaCode = r.ns[0].ns[0].c;
            this.area = r.ns[0].ns[0].n;
          }
          this.provinceCode = r.c;
          this.province = r.n;
          // ----------------
          this.provinceData.forEach((el, index) => {
            if (this.provinceCode === el.c) {
          ...
          // ----------------
          this.$nextTick(() => {
            _this.topDealMethod(this.provinceIndex, 'provinceRef');
          });
        });
      } else {
        // 不做回显
