...
export default class IndexedDBLogger extends LoggerInterface {
   ...
   */
  async _record(level, descriptor, data) {
    try {
      const uid = await commonPara.uid();
  ...
  static init(database) {
    try {
      ...
      IndexedDBLogger.request.onupgradeneeded = event => {
        ...
        store.createIndex('curUrl', 'curUrl', {
          unique: false
        });
      };
    } catch (e) {
        ...
        request.onsuccess = event => {
          ...
          if (cursor) {
            ...
            let log = {
              ...
              deviceId: cursor.value.deviceId,
            ...
            let cursorCurUrl = cursor.value.curUrl;
            cursorCurUrl ? log['curUrl'] = cursorCurUrl : '';
