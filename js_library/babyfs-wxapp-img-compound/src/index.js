...
// system
const SYSTEM = wapi.getSystemInfoSync().system;

const android = /android/ig;
const delayTime = 1000;
let isAndroid = false;
let t = null; // BUG: andriod合成二维码无法识别 延时合成
 ...
 * @param {Object} e 组件实例this
 * @returns canvas ctx
 */
function getInstance(canvasId, e) {
  if (!canvasId) {
    throw new Error('canvasid is required');
  }
  return wapi.createCanvasContext(canvasId, e);
};
 ...
 */
const _draw = async function (canvasId = '', argv = {}, e) {
  if (!isAndroid) {
    return new Promise((resolve, reject) => {
      this.draw(Boolean(argv['reserve']), async () => {
        wapi.canvasToTempFilePath({
          ...argv,
          canvasId,
          success(res) {
            resolve(res);
 ...
 * @param {Object} e 当前组件实例this，页面不用传此参数
 */
const compound = async function (canvasId, options, config, e) {
  const ctx = getInstance(canvasId, e);
  ...
  let r = await _draw.call(ctx, canvasId, config, e);
