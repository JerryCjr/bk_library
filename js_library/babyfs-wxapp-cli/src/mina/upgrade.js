...
async function copyOthers(dirPath, options) {
  ...
  if (options.force || checkOverride('other config files', override)) {
    ...
    const originVScodeFilePath = path.join(dirPath, '.vscode.js');
    await _.removeDir(originVScodeFilePath);
    vscodefiles = await _.globSync('.vscode/**/*', globOptions);

    otherConfig = [...eslint, ...editor, ...vscodefiles];
  }
