...
// const API_HOST = '//wapi.dev.babyfs.cn/api'; // host
...
function traceUpload() {
  ...
  assert(arg[1] instanceof Array, 'Type error, should be array');
  const logs = [...arg[1], {
    ua: window.navigator.userAgent.toString()
  }];
  const FILE = new Blob([JSON.stringify(logs, null, 0)], { type: 'application/json' });
  ...
  const CONFIG = {
    ...
    token: 'AyS1gYvZtmwvi9Jr1hCznQgxtYHlfL1B3Cn0eR-L:7mtc_Mnj29Bv3_3CHGCJ_cWOyUk=:eyJkZWxldGVBZnRlckRheXMiOjEsInJldHVybkJvZHkiOiJ7XCJrZXlcIjpcIiQoa2V5KVwiLFwiaGFzaFwiOlwiJChldGFnKVwiLFwiZnNpemVcIjokKGZzaXplKSxcImJ1Y2tldFwiOlwiJChidWNrZXQpXCIsXCJuYW1lXCI6XCIkKHg6bmFtZSlcIn0iLCJzY29wZSI6InN0YXRpYyIsImRlYWRsaW5lIjoxNTg2OTMzNDMwfQ==',
    putExtra: {
 ...
 * @param {blob/array} _file _utype === UTYPE.trace type: array, otherwise blob
 * @param {string} _key
 * @param {string} _token
